  ...
  <div class="space-y-8 animate-fade-in">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        ...
        <p class="text-slate-500 mt-1">Générez un plan personnalisé par l'IA selon votre équipement</p>
      </div>
    ...
    <div class="card">
      ...
      <div class="mt-6 pt-5 border-t border-slate-100 flex items-center justify-between gap-4">
        <p class="text-sm text-slate-500">
          ...
          · {{ goalLabels[userStore.selectedUser?.goal] }}
          · {{ form.daysPerWeek }}j/semaine
        </p>
    ...
    <!-- État vide -->
    <div v-if="!plan && !generating && !error" class="flex flex-col items-center justify-center py-16 text-center">
      <div class="w-20 h-20 rounded-2xl bg-brand-neutral border border-slate-200 flex items-center justify-center mb-5">
        <svg class="w-10 h-10 text-slate-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round">
          <path d="M6 4v16M18 4v16M10 8H6M10 16H6M18 8h-4M18 16h-4"/>
        </svg>
      </div>
      <p class="text-brand-secondary font-medium">Configurez vos paramètres et générez votre programme</p>
      <p class="text-slate-400 text-sm mt-1">Le programme sera adapté à votre objectif et équipement</p>
    </div>
  </div>
</template>
...
async function generatePlan() {
  ...
  generating.value = true; error.value = ''; plan.value = null
  try {
    const res = await coachAPI.getWorkoutPlan(userStore.selectedUserId, form.value.equipment, form.value.daysPerWeek)
    plan.value = res.data
  } catch {
    error.value = "Erreur lors de la génération du programme. Vérifiez la connexion à l'API."
  } finally {
